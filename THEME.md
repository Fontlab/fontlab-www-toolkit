...
# Shared theme integration: fontlab-www-toolkit

## Shared hybrid-site build pipeline

The Python toolkit injects the shared assets into final HTML after Markdown generation, Webflow cache overlay and static-file overlay. Its own documentation is an internal consumer, not an independently configured Pages site.

Ownership is chosen by each consumer. Preserve both global and MaterialX options; the main FontLab/Vexy hybrid sites explicitly select global controls.

## Configuration and destinations

| Configuration | Role | Destination |
|---|---|---|
| `src_docs/mkdocs.yaml` | internal documentation | source/internal build only |

## Build and publish responsibility

Keep the final-output injection idempotent: one copy of each shared asset, in the documented order. Run the toolkit tests and build an actual hybrid consumer when changing injection. Publishing README/AGENTS/THEME does not require a new Python package release; executable package changes follow the existing package release procedure.
